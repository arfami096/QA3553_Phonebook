...
class TestRemoveContact:
  ...
  def test_remove_contact(self, authenticated_driver, created_contact):
    ...
    # Используем контакт, который создала фикстура из conftest.py
    ...
    with allure.step("Verify the contact is successfully removed"):
      assert contacts_page.wait_until_contact_disappears(contact.phone), (
          f"Ошибка: контакт с телефоном {contact.phone} не был удален из списка!"
      )